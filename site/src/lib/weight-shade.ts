// The ribbon's sequential ramp: one amber hue, light to dark, so a slat's
// shade restates the weight its width already draws. Three stops — a pale
// wash, the brand marigold, a deep amber — interpolated in oklab, where a
// straight line between stops keeps perceived lightness moving one way.
// Computed at build time so every slat ships a plain hex, no runtime color
// math and no color-mix() support question.
